  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 text-left">
      ...
      <div className="border border-[#D8DCD4] bg-white p-4 mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        {/* Explicit Cross-cover grant notification if any */}
        {activeGrantsForUser.length > 0 && (
          <div className="bg-[#E6F0EA] border border-[#2F6F4E] p-2 text-xs font-mono text-[#14213D] flex items-center gap-2">
      ...
      {/* Hidden records notification required by spec */}
      {hiddenCount > 0 && !showAllHospitalWards && (
        <div className="mb-4 bg-[#F3F5F1] border border-[#D8DCD4] px-4 py-2.5 text-xs text-[#5B6470] flex items-center justify-between">
          <div className="flex items-center gap-2">
      ...
      {showAllHospitalWards && (
        <div className="mb-4 bg-[#FBF1E3] border border-[#D98E2A] px-4 py-2.5 text-xs text-[#14213D] flex items-center justify-between">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          <Filter className="w-3.5 h-3.5 text-[#5B6470]" />
          <span className="text-[#5B6470]">Ward:</span>
          {(['All', 'Cardiology', 'Pediatrics', 'Emergency', 'General Medicine'] as const).map((ward) => (
            <button
              key={ward}
      ...
      {/* Off-shift warning if applicable */}
      {currentUser?.dutyStatus === 'off shift' && (
        <div className="bg-[#FBEAE7] border-l-4 border-[#B23A2E] p-4 mb-4 text-xs">
      ...
      <div className="border border-[#D8DCD4] bg-white divide-y divide-[#D8DCD4]">
        {/* Table Header */}
        <div className="bg-[#F3F5F1] px-4 py-2 text-[11px] font-mono uppercase text-[#5B6470] grid grid-cols-12 gap-2">
          <div className="col-span-1">Status</div>
          <div className="col-span-4 sm:col-span-3">Patient</div>
          <div className="col-span-2 hidden sm:block">Ward / Bed</div>
          <div className="col-span-3 hidden md:block">Active Conditions</div>
        ...
        {filteredInScope.length > 0 ? (
          filteredInScope.map(({ patient, reason, viaOverride, viaGrant }) => (
            <div
              key={patient.id}
              className={`px-4 py-3 grid grid-cols-12 gap-2 items-center hover:bg-[#FAFAF8] transition-colors ${
                viaOverride
            ...
            >
              ...
              <div className="col-span-4 sm:col-span-3">
                <div className="font-semibold text-xs text-[#14213D]">{patient.name}</div>
                <div className="text-[11px] font-mono text-[#5B6470] flex items-center gap-1.5">
                  <span>{patient.id}</span>
                  {patient.isSensitive && (
                    <span className="text-[#D98E2A] font-semibold text-[10px] uppercase">
                      &bull; Sensitive
                    </span>
                  )}
                </div>
              ...
              <div className="col-span-2 hidden sm:block text-xs font-mono">
                ...
                <span className="text-[#5B6470] text-[11px]">{patient.roomBed}</span>
              </div>

              {/* Conditions */}
              <div className="col-span-3 hidden md:block text-xs text-[#5B6470] truncate">
                <span>{patient.majorConditions[0]}</span>
                {patient.majorConditions.length > 1 && (
                  <span className="text-[10px] text-[#5B6470] ml-1">
        ...
        {showAllHospitalWards &&
          filteredOutOfScope.map(({ patient, reason }) => (
            <div
              key={patient.id}
              className="px-4 py-3 grid grid-cols-12 gap-2 items-center bg-[#F3F5F1]/50 border-l-4 border-[#5B6470]"
            >
              <div className="col-span-1 flex items-center">
                <Lock className="w-3.5 h-3.5 text-[#5B6470]" />
              </div>
              ...
              <div className="col-span-4 sm:col-span-3 opacity-80">
                <div className="font-semibold text-xs text-[#14213D]">{patient.name}</div>
                <div className="text-[11px] font-mono text-[#5B6470]">{patient.id}</div>
              </div>

              <div className="col-span-2 hidden sm:block text-xs font-mono text-[#5B6470]">
                <span>{patient.ward}</span>
                <span className="text-[11px] block">{patient.roomBed}</span>
              </div>

              <div className="col-span-3 hidden md:block text-xs text-[#5B6470] italic">
              ...
              <div className="col-span-7 sm:col-span-4 md:col-span-3 lg:col-span-2 text-right">
                <button
                  ...
                  className="px-2.5 py-1 border border-[#D98E2A] text-[#14213D] hover:bg-[#FBF1E3] text-[11px] font-mono font-semibold flex items-center gap-1 ml-auto cursor-pointer"
                >
